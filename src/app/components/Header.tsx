...
export default function Header() {
  ...
  return (
    <header className="bg-[#F0F0E6] shadow-lime-100 text-black">
      <div className="container mx-auto flex items-center justify-between px-4 md:px-0">
        <Image className="w-16" src={logo} alt="logo" />
        <nav className={`${isOpen ? "block" : "hidden"} md:block space-x-8`}>
          <ul className="flex items-center text-lg justify-center space-x-8">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-white p-4 rounded-lg shadow-lg w-full md:w-64 h-full">
            ...
            <ul className="space-y-4 font-medium">
              ...
              <li>
                <button
                  className="border-black p-2  border rounded-full text-sm"
                  onClick={() => {
                    window.open(
                      "https://www.linkedin.com/company/disruptorsden/",
                      "_blank"
